...
def main():
    event_date = "07-10-2022 22:09"
    arrival_object = create_time_object(event_date)

    while True:
        # clear the terminal on every loop
        os.system("clear")
        
        remaining_time = time_till_wow(arrival_object)
        print("\n" + remaining_time)

        # update remaining time on every second
        time.sleep(1)
...
def time_till_wow(date):
    ...
    return f"In {rest_days} days, {rest_hours} hours, {rest_minutes} minutes and {rest_seconds} seconds."
